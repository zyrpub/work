...
def cnn3d(inputs, param_dict, train, name='cnn3d'): # no dropout at last layer!!! # (None, 4, 50)
	...
	with tf.variable_scope(name, reuse=reusing) as scope:
		...
		# [None, 4, 25, 60, 1]
		sensor_conv1 = layers.convolution2d(inputs, convNum1, kernel_size=[1, 1, 6],
						stride=[1, 1, 2], padding='VALID', data_format='NDHWC', scope=SCOPE_CONV%scnt(1),activation_fn=None)
		#(None, 4, 25, 30, 16)
		sensor_conv1 = batch_norm_layer(sensor_conv1, train, scope=SCOPE_NORM%scnt())
		sensor_conv1 = activation_layer(sensor_conv1)
		...
		sensor_conv1 = layers.dropout(sensor_conv1, CONV_KEEP_PROB, is_training=train,
			noise_shape=[sensor_conv1_shape[0], 1, 1, 1, sensor_conv1_shape[4]], scope=SCOPE_DROP%scnt())
		print(scnt(),sensor_conv1_shape)


		padding = "SAME"
		sensor_conv2 = layers.convolution2d(sensor_conv1, convNum2, kernel_size=[1, 1, 4],
		...
		sensor_conv2 = layers.dropout(sensor_conv2, CONV_KEEP_PROB, is_training=train, 
			noise_shape=[sensor_conv2_shape[0], 1, 1, 1, sensor_conv2_shape[4]], scope=SCOPE_DROP%scnt())
		print(scnt(),sensor_conv2_shape)


		sensor_conv3 = layers.convolution2d(sensor_conv2, convNum3, kernel_size=[1, sensor_conv2_shape[2], 1],
						stride=[1, 1, 1], padding='VALID', data_format='NDHWC', scope=SCOPE_CONV%scnt(1),activation_fn=None)
		# (None, 4, 1, 15, 16)
		sensor_conv3 = batch_norm_layer(sensor_conv3, train, scope=SCOPE_NORM%scnt())
		sensor_conv3 = activation_layer(sensor_conv3)
		sensor_conv3_shape = sensor_conv3.get_shape().as_list()
		sensor_conv3 = layers.dropout(sensor_conv3, CONV_KEEP_PROB, is_training=train, 
			noise_shape=[sensor_conv3_shape[0], 1, 1, 1, sensor_conv3_shape[4]], scope=SCOPE_DROP%scnt())
		print(scnt(),sensor_conv3_shape)


		conv = layers.convolution2d(sensor_conv3, convNum4, kernel_size=[1, 1, 3],
						stride=[1, 1, 2], padding=padding, data_format='NDHWC', scope=SCOPE_CONV%scnt(1),activation_fn=None)
		# (None, 4, 1, 7, 16)
		conv = batch_norm_layer(conv, train, scope=SCOPE_NORM%scnt())
		conv = activation_layer(conv)
