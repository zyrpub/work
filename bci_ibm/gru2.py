...
def gru2(inputs, param_dict={}, train=True, name='gru2'): # [None, rnnDim]
	...
	with tf.variable_scope(name, reuse=reusing) as scope:
		...
		seq_length = tf.reduce_sum(seq_length, axis=1, keep_dims=False)
		...
		avgNum = tf.reduce_sum(avgNum, axis=1, keep_dims=False)
		...
		sum_cell_out = tf.reduce_sum(cell_output, axis=1, keep_dims=False) #*mask
