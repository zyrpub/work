...
def cnn2d_pool_deep(inputs, param_dict={}, train=True, name='cnn2d_pool_deep'): # out (None, 100)
	convNum1 =param_dict["convNum1"]
	convNum2 =param_dict["convNum2"]
	convNum3 =param_dict["convNum3"]
	convNum4 =param_dict["convNum4"]
	...
	with tf.variable_scope(name, reuse=reusing) as scope:
		...
		conv = layers.convolution2d(inputs, convNum1, kernel_size=[1, 10],stride=[1, 1], padding='VALID', data_format='NHWC', scope=SCOPE_CONV%scnt(1),activation_fn=None)
		#(None, 25, 366, 16)
		conv = batch_norm_layer(conv, train, scope=SCOPE_NORM%scnt())
		conv = activation_layer(conv)
		...
		conv = layers.convolution2d(conv, convNum1, kernel_size=[1, 6],stride=[1, 1], padding='VALID', data_format='NHWC', scope=SCOPE_CONV%scnt(1),activation_fn=None)
		#(None, 25, 178, 16)
		conv = batch_norm_layer(conv, train, scope=SCOPE_NORM%scnt())
		conv = activation_layer(conv)
		conv = tf.nn.max_pool(conv,pooling,pooling,padding="SAME",data_format='NHWC',name=SCOPE_POOL%scnt()) 
