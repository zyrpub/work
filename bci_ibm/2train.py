...
def pred_loss_ensemble(last_layer, out_W, labels):
	with tf.variable_scope('ensemble'):
		...
		out_W = tf.tile(out_W, [last_layer.get_shape().as_list()[0],1,1]) # (None, 100, 4)
		...
		batch_predictions=[]
		for i in range(ensemble_predict.get_shape().as_list()[0]): # per sample, has to be 1-D.
...
lg.lg_list(["inputDim=",inputDim])
lg.lg_list(["CONV_KEEP_PROB=",CONV_KEEP_PROB])
lg.lg_list(["OUT_DIM=",OUT_DIM])
...
from cnn2d import cnn2d,cnn2d_pool,cnn_pool_split,cnn2d_pool_deep
...
with tf.variable_scope('input_layer',reuse=False):
	inputs = layers.fully_connected(inputs, inputDim, activation_fn=activation_layer, scope='input')
	inputs_shape = inputs.get_shape().as_list()
	print('input transpose fully_connected',inputs_shape)
inputs = tf.transpose(inputs, perm=[0,1,3,2]) # [None,1,height,width]

cnn_out = cnn_pool_split(inputs, cnn_param_dict, is_training) #(None, ens, 100)
...
predict, loss_ent= pred_loss_ensemble(last_layer, out_W, labels)
...
predict_t, loss_ent_t= pred_loss_ensemble(last_layer, out_W, labels)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=False,allow_soft_placement=True)) as sess:
	...
	while trainFeed.get_epoch()<nepoch:
		...
		if iteration % 50 == 0:
			...
			print("epoch",trainFeed.get_epoch(),"batch",iteration,"lr %.4f"%lr_val[0])
			trainloss = lossV
			trainacc = _accuracy
		...
		if trainFeed.get_epoch()>testcnt:
			...
			print(My_IP,'-- test l=',np.mean(dev_cross_entropy),"a=",np.mean(dev_accuracy))
			print(dataname,"subj",ind2trainSubj[dataName2ind[dataname]],ind2testSubj[dataName2ind[dataname]])
	...
	for eval_idx in xrange(int(test_size/batch_size)):
		...
		dev_accuracy.append(_accuracy)
		dev_cross_entropy.append(eval_loss_v)
	print('-----test loss',np.mean(dev_cross_entropy),"accuracy",np.mean(dev_accuracy))
	testloss = np.mean(dev_cross_entropy)
	testacc = np.mean(dev_accuracy)
