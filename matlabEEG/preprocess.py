...
# sys.path.append("/Users/yiran.zhaoibm.com/Dropbox/zyr/mytools")
# sys.path.append("/Users/yiranzhao/Dropbox/zyr/mytools")
...
class preProcessor:
	...
	def change_sample_rate(self, x0, fromrate, torate):
		...
		print(newlen,self.a0,len(self.a0),self.a1,len(self.a1))
	...
	def add_noise(self,x,nr=0.1): # add noise ratio 
		x = np.asarray(x)
		stdd=x.std(axis=-1)
		amp = abs(nr*stdd)
		return x+ np.random.normal(0.0,amp,size=x.shape)

	def drop_out(self,x,ratio=0.1): # consecutive seg drop to zero 
		x = np.asarray(x).reshape(1,-1)
	...
	def drop_zero_sparse(self,x,ratio=0.1):
		x = np.asarray(x).reshape(1,-1)
		dropsz = int(x.shape[-1]*ratio)
		while dropsz>0:
			dropsz-=1
